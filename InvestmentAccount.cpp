...
// Method to calculate the ending balance
// @return balance after x amount of years with an initial deposit
...
double InvestmentAccount::getEndingBalance(int numberOfYears) {
	...
	for (int i = 0; i < numberOfYears * 12; i++) {
		double interest = (total + monthlyDeposit) * monthlyInterestRate;
		total += interest + monthlyDeposit;
	}
	return total;
}

// Method to calculate the interest earned
// @return interest earned after x amount of years
// @param number of years
double InvestmentAccount::getInterestEarned(int numberOfYears) {
	double balance = 0.0;
	for (int i = 0; i < numberOfYears; i++) {
		balance = (getEndingBalance(i + 1) - getEndingBalance(i)) - (12 * monthlyDeposit);
	}
	return balance;
}
...
std::string InvestmentAccount::getInterestEarnedFormatted(int numberOfYears) {
	double endingInterest = getInterestEarned(numberOfYears);

	return format(round(endingInterest, 2));
}
...
std::string InvestmentAccount::getEndingBalanceFormatted(int numberOfYears) {
	...
	return format(round(endingBalance, 2));
}
